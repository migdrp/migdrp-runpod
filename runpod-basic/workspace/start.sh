...
cleanup_stale_processes() {
    ...
    for port in 8888 7860; do # Only check Jupyter and ttyd ports
    ...
    # Clean up Python cache files (optional)
...
setup_workspace() {
    ...
    if [ -d "/workspace_template" ]; then
        ...
        find /workspace_template -maxdepth 1 -type f -print0 | while IFS= read -r -d $'\0' file; do
            ...
            # Only copy .sh files (or other necessary scripts)
            if [[ "$basename" == *.sh ]] && [ ! -e "/workspace/$basename" ]; then
...
# --- Main Execution ---
main() {
    log "${COLOR_GREEN}Starting container initialization (basic)...${COLOR_RESET}"
